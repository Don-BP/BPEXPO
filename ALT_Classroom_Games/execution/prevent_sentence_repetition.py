...
const SentenceScramble = () => {
    ...
    // Track completed sentences to avoid repetition
    const [completedSentences, setCompletedSentences] = useState([]);

    // Reset completed history when level changes
    useEffect(() => {
        setCompletedSentences([]);
        // We don't call initGame here directly because the useEffect below will trigger due to level change? 
        // Actually, let's leave the separate useEffect for initGame or combine.
        // The original code had: useEffect(() => { initGame(); }, [level]);
        // We should clear history THEN init.
    }, [level]);

    // Initialize game when level changes (and logic inside handles history)
    useEffect(() => {
        // We need to wait for completedSentences to be cleared? 
        // No, the previous effect runs, then component re-renders... actually React batches state updates might be tricky.
        // Let's rely on initGame logic.
        initGame(true); // Pass flag to forced-reset or just pick new.
    }, [level]);

    const initGame = (resetHistory = false) => {
        const allSentences = LEVELS[level];
        let history = resetHistory ? [] : completedSentences;
        
        // If we reset history via argument, update state
        if (resetHistory) {
            setCompletedSentences([]);
        }
        ...
        <div className="h-screen w-full relative overflow-hidden font-sans text-white flex flex-col items-center">
            ...
            <header className="flex-none w-full flex items-center justify-between px-6 py-4 z-20 bg-slate-900/30 backdrop-blur-md border-b border-white/5">
                ...
                <div className="flex gap-4">
                    ...
                    <button onClick={() => initGame(true)} className="p-3 bg-white/5 rounded-full hover:bg-white/10 transition hover:scale-105 border border-white/5">
...
print(f"SUCCESS: Implemented non-repeating sentences in {target_file}")
