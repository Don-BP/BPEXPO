...
export function initImageReveal() {
    ...
    if (irTool.dataset.initialized) return;
    irTool.dataset.initialized = "true";
    ...
    function createAndStartGrid() {
        ...
        if (irRevealModeSelect.value === 'manual') {
            ...
            clearInterval(irRevealInterval); // Safety check
    ...
    irStartPauseBtn.addEventListener('click', () => {
        ...
        else if (irGameState === 'paused') { irGameState = 'playing'; irStartPauseBtn.textContent = 'Pause'; irGameStatus.textContent = 'Revealing automatically...'; let speed = irRevealModeSelect.value === 'auto' ? 2000 : (parseFloat(irCustomSpeedInput.value) || 2) * 1000; clearInterval(irRevealInterval); irRevealInterval = setInterval(revealOneTile, speed); }
    });
