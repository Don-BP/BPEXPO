...
export function initWhatsMissing() {
    ...
    if (toolCard.dataset.initialized) return;
    toolCard.dataset.initialized = "true";
    ...
    let wmTimer = null; // Add timer reference to clear later
    ...
    async function wmStartGame() {
        if (wmGameState === 'showing' || wmGameState === 'hiding') return; // Prevent double trigger
        wmStartBtn.disabled = true; // Disable immediately
        clearTimeout(wmTimer); // Clear any pending timer from previous game
        let cardCount;
