...
export function initTimer() {
    ...
    const waterDripContainer = document.getElementById('water-drip-container');
    ...
    function startPhysics(gravityY = 1) {
        ...
        const { Engine, Runner, World, Bodies, Composite } = Matter;
        physicsEngine = Engine.create({ gravity: { y: gravityY } });
        ...
        return { World, Bodies, Body: Matter.Body };
    }
    ...
    function stopPhysics() {
        ...
        if (marblesContainer) marblesContainer.innerHTML = '';
        if (waterDripContainer) waterDripContainer.innerHTML = '';
    }
    ...
    function resetAllThemeStates() {
        ...
        if (waterDripContainer) {
            // Clear all potentially conflicting inline styles for a clean state
            waterDripContainer.style.width = '';
            waterDripContainer.style.height = '';
            waterDripContainer.style.transform = '';
            waterDripContainer.style.position = '';
            waterDripContainer.style.left = '';
            waterDripContainer.style.top = '';
        }
    ...
    function updateTimerDisplay() {
        ...
        if (selectedTheme === 'bar') { 
        ...
        else if (selectedTheme === 'water-drip') {
            if (!isTimerRunning || !complexTimerData.particleBodies) return;

            const totalParticles = complexTimerData.totalParticles;
            const particlesThatShouldBeGone = Math.floor(totalParticles * (percentageDone / 100));
            const numToRemove = particlesThatShouldBeGone - complexTimerData.particlesRemovedSoFar;

            if (numToRemove > 0) {
                complexTimerData.particleBodies.sort((a, b) => a.position.y - b.position.y);
                
                for (let i = 0; i < numToRemove; i++) {
                    if (complexTimerData.particleBodies.length === 0) break;

                    const particleToVanish = complexTimerData.particleBodies.shift();
                    if (particleToVanish) {
                        Matter.World.remove(physicsEngine.world, particleToVanish);
                        if (particleToVanish.domElement) {
                            particleToVanish.domElement.style.transition = 'opacity 0.2s ease-out';
                            particleToVanish.domElement.style.opacity = '0';
                            setTimeout(() => particleToVanish.domElement.remove(), 250);
                        }
                    }
                    ...
                    const radius = 5;
                    dripEl.style.width = `${radius * 2}px`;
                    dripEl.style.height = `${radius * 2}px`;
                    waterDripContainer.appendChild(dripEl);

                    const { x, y } = complexTimerData.spoutPosition;
                    dripEl.style.left = `${x - radius}px`;
                    dripEl.style.top = `${y}px`;
                    
                    setTimeout(() => dripEl.remove(), 1200);
                }
                complexTimerData.particlesRemovedSoFar += numToRemove;
            }
        }
            ...
            if (sceneEl) {
                ...
                if (isMobileFullscreen) {
                    ...
                    const scale = Math.min(1.0, containerHeight / designHeight, containerWidth / designWidth);
                    sceneEl.style.setProperty('--flower-scene-scale', scale);
                } else {
                    sceneEl.style.setProperty('--flower-scene-scale', 1.0);
                }
            }
            const STEM_END_PERCENT = 60, LEAF1_START_PERCENT = 10, LEAF1_END_PERCENT = 50, LEAF2_START_PERCENT = 20, LEAF2_END_PERCENT = 60, HEAD_START_PERCENT = 55, HEAD_END_PERCENT = 75, PETALS_START_PERCENT = 70, PETALS_END_PERCENT = 100;
            ...
            if (marblesToRemove > 0) {
                for (let i = 0; i < marblesToRemove; i++) {
                    if (marbleBodies.length > 0) {
                        ...
                        Matter.World.remove(physicsEngine.world, marbleBody);
            ...
            // All pug logic is handled by the pugEatNextFood() chain.
        }
    }
    ...
    function triggerEndAnimation() {
        ...
        } else if (selectedTheme === 'marbles' || selectedTheme === 'water-drip') {
            if (complexTimerData.particleBodies) {
                complexTimerData.particleBodies.forEach(body => {
                    if (body.domElement) {
                        body.domElement.style.transition = 'opacity 0.2s';
                        body.domElement.style.opacity = '0';
                    }
                });
                setTimeout(() => {
                    complexTimerData.particleBodies.forEach(body => body.domElement?.remove());
                    complexTimerData.particleBodies = [];
                }, 200);
            }
        }
    }
    ...
    function setupComplexTimers() {
        ...
        if (selectedTheme === 'marbles') {
            const { World, Bodies } = startPhysics(1);
            ...
            World.add(physicsEngine.world, [ground, leftWall, rightWall]);
            ...
            for (let i = 0; i < numMarbles; i++) {
                ...
                marbleDOM.className = 'physics-particle marble';
                ...
                const marbleBody = Bodies.circle(radius + Math.random() * (scene.clientWidth - radius * 2), radius + Math.random() * (scene.clientHeight / 2), radius, { restitution: 0.01, friction: 0.1, slop: 0.0000001 });
                marbleBody.domElement = marbleDOM;
                marbleBody.domRadius = radius;
                complexTimerData.marbleBodies.push(marbleBody);
            }
            World.add(physicsEngine.world, complexTimerData.marbleBodies);

        } else if (selectedTheme === 'water-drip') {
            waterDripContainer.style.opacity = '0';

            const { World, Bodies } = startPhysics(0.8);
            waterDripContainer.innerHTML = '';
            complexTimerData.particleBodies = [];
            
            const scene = document.querySelector('.water-drip-scene');
            const containerWidth = 300; 
            const containerHeight = 420;
            
            waterDripContainer.style.width = `${containerWidth}px`;
            waterDripContainer.style.height = `${containerHeight}px`;

            const centerX = containerWidth / 2;
            const jarInnerWidth = 260;
            const jarHeight = 380;
            const wallThickness = 40;
            const wallAngle = 0.15;
            const floorY = containerHeight / 2 + jarHeight / 2 - wallThickness / 2;

            const wallHeight = jarHeight;
            const leftWall = Bodies.rectangle(centerX - jarInnerWidth / 2 - wallThickness / 2, containerHeight / 2, wallThickness, wallHeight, { isStatic: true, angle: -wallAngle });
            const rightWall = Bodies.rectangle(centerX + jarInnerWidth / 2 + wallThickness / 2, containerHeight / 2, wallThickness, wallHeight, { isStatic: true, angle: wallAngle });
            const solidFloor = Bodies.rectangle(centerX, floorY, jarInnerWidth + wallThickness, wallThickness, { isStatic: true });
            
            complexTimerData.spoutPosition = { x: centerX, y: floorY + (wallThickness / 2) - 10 };
            
            World.add(physicsEngine.world, [leftWall, rightWall, solidFloor]);
            
            const numParticles = 1000;
            complexTimerData.totalParticles = numParticles;
            complexTimerData.particlesRemovedSoFar = 0;
            const radius = 5;

            for (let i = 0; i < numParticles; i++) {
                const particleDOM = document.createElement('div');
                particleDOM.className = 'physics-particle water';
                particleDOM.style.width = `${radius * 2}px`;
                particleDOM.style.height = `${radius * 2}px`;
                waterDripContainer.appendChild(particleDOM);
                
                const spawnX = centerX + (Math.random() - 0.5) * (jarInnerWidth * 0.9);
                const spawnY = (containerHeight - jarHeight) / 2 + (Math.random() * jarHeight * 0.7);
                
                const particleBody = Bodies.circle(spawnX, spawnY, radius, { 
                    restitution: 0.9,
                    friction: 0.000001,
                    slop: 0.000001,
                ...
                particleBody.domElement = particleDOM;
                particleBody.domRadius = radius;
                complexTimerData.particleBodies.push(particleBody);
            }
            World.add(physicsEngine.world, complexTimerData.particleBodies);

            // === START FIX ===
            const isFullscreen = scene.closest('.tool-card.fullscreen-mode');

            // Reset any conflicting inline styles from previous states.
            waterDripContainer.style.transform = '';
            waterDripContainer.style.position = '';
            waterDripContainer.style.left = '';
            waterDripContainer.style.top = '';
            ...
            if (!isFullscreen) {
                // In GRID VIEW, we must manually position AND scale the container.
                waterDripContainer.style.position = 'absolute';
                waterDripContainer.style.left = '50%';
                waterDripContainer.style.top = '50%';
                // translateX(-50%) centers horizontally.
                // translateY(-45%) moves it slightly down from the perfect center.
                // scale(0.65) makes it smaller to fit the grid view jar.
                waterDripContainer.style.transform = 'translate(-50%, -10%) scale(0.50)';
            }
            // In FULLSCREEN, no styles are applied, so it will be centered by its parent's CSS flex properties.
            // === END FIX ===

            setTimeout(() => {
                waterDripContainer.style.opacity = '1';
                waterDripContainer.style.transition = 'opacity 0.3s';
            }, 500);
            ...
            for (let i = 0; i < numFood; i++) {
                ...
                complexTimerData.foodItems.push({ element: food, pos: { x: food.offsetLeft + food.offsetWidth / 2, y: food.offsetTop + food.offsetHeight / 2 }, eaten: false, foodType: foodType });
            }
            ...
            complexTimerData.pugPos = { x: (pugRect.left - containerRect.left) + (pugRect.width / 2), y: (pugRect.top - containerRect.top) + (pugRect.height / 2) };
            pugDog.style.backgroundImage = `url('assets/pug_timer/pug-idle/idle-frame1.png')`;
        }
    }
    ...
    timerThemeSelect.addEventListener('change', () => {
        ...
        resetAllThemeStates();
        const selectedTheme = timerThemeSelect.value;
        const sideBySideThemes = ['water-drip', 'flower', 'marbles', 'pug'];
