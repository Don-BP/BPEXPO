...
import {
    ...
    getCategoryCards,
    getUserSetCards
} from './tango-data-bridge.js';
import { getAllTangoCardMetadata } from './db.js';
...
/**
 * Retrieves all flashcard decks, combining built-in, custom from DB, Tango categories, and the live edited one.
...
export async function getAvailableFlashcardDecks() {
    ...
    // 2. Add Tango categories
    const tangoCategories = await getTangoCategories();
    for (const category of tangoCategories) {
        const cards = await getCategoryCards(category);
        const cardsWithMetadata = await applyTangoMetadata(cards);
        allDecks[`Tango: ${category}`] = cardsWithMetadata;
    }

    // 3. Add user-created Tango sets
    const userSets = getUserCreatedSets();
    for (const set of userSets) {
        const cards = await getUserSetCards(set.id);
        const cardsWithMetadata = await applyTangoMetadata(cards);
        allDecks[`Tango: ${set.name}`] = cardsWithMetadata;
    }

    // 4. If a deck is being edited live, overwrite the version from storage.
...
/**
 * Applies Tango metadata (muted state) to Tango cards
 * @param {Array} cards - Array of Tango cards
 * @returns {Promise<Array>} Cards with muted metadata applied
 */
async function applyTangoMetadata(cards) {
    const metadata = await getAllTangoCardMetadata();
    return cards.map(card => ({
        ...card,
        muted: card.tangoId && metadata[card.tangoId] ? metadata[card.tangoId].muted : false
    }));
}
...
export async function updateAllFlashcardCategorySelects() {
    ...
    selects.forEach(selectElement => {
        ...
        // Add custom sets from IndexedDB and Tango sets
        const customDecks = Object.keys(allDecks).filter(k => !builtInFlashcardData.hasOwnProperty(k));
        const tangoDecks = customDecks.filter(k => k.startsWith('Tango: '));
        const regularCustomDecks = customDecks.filter(k => !k.startsWith('Tango: '));

        // Add regular custom sets
        if (regularCustomDecks.length > 0) {
            const optGroup = document.createElement('optgroup');
            optGroup.label = 'My Custom Sets';
            for (const category of regularCustomDecks) {
                const option = document.createElement('option');
                option.value = category;
                option.textContent = category;
        ...
        // Add Tango sets
        if (tangoDecks.length > 0) {
            const tangoOptGroup = document.createElement('optgroup');
            tangoOptGroup.label = 'Tango Vocabulary';
            for (const category of tangoDecks) {
                const option = document.createElement('option');
                option.value = category;
                option.textContent = category;
                tangoOptGroup.appendChild(option);
            }
            selectElement.appendChild(tangoOptGroup);
        }
