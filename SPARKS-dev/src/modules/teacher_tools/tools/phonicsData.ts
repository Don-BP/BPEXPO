export interface PhonicsCard {
  word: string;     // HTML with <strong> tags highlighting the focus grapheme
  image: string;    // relative path, e.g. "assets/phonics/apple.png"
  sentence: string;
}

export interface PhonicsSubgroup {
  name: string;
  focus: string;    // comma-separated graphemes
  cards: PhonicsCard[];
}

export interface PhonicsCategory {
  name: string;
  subgroups: Record<string, PhonicsSubgroup>;
}

export const phonicsData: Record<string, PhonicsCategory> = {
  phase0_alphabet: {
...
// Grapheme teaching order — used for sorting deck cards
const masterGraphemeOrderMap = new Map<string, number>();
let orderIndex = 0;
Object.values(phonicsData).forEach(phase => {
  Object.values(phase.subgroups).forEach(sub => {
    sub.focus.split(',').forEach(g => {
      const t = g.trim();
      if (!masterGraphemeOrderMap.has(t)) masterGraphemeOrderMap.set(t, orderIndex++);
    });
  });
});

function getFocusGrapheme(card: PhonicsCard): string | null {
  const m = card.word.match(/<strong>(.*?)<\/strong>/);
  return m ? m[1] : null;
}

export function buildDeck(checkedKeys: string[], isCumulative: boolean): PhonicsCard[] {
  const cardSets = new Set<PhonicsCard[]>();
  ...
  if (isCumulative) {
    Object.entries(phonicsData).forEach(([catKey, cat]) => {
      ...
      subKeys.forEach((subKey, idx) => {
        if (checkedKeys.includes(`${catKey}|${subKey}`)) highestIdx = idx;
      });
      if (highestIdx !== -1) {
        for (let i = 0; i <= highestIdx; i++) {
          cardSets.add(cat.subgroups[subKeys[i]].cards);
        }
      }
    });
  } else {
    checkedKeys.forEach(key => {
      const [catKey, subKey] = key.split('|');
      const cards = phonicsData[catKey]?.subgroups[subKey]?.cards;
      if (cards) cardSets.add(cards);
    });
  }

  const deck: PhonicsCard[] = [];
  cardSets.forEach(arr => deck.push(...arr));

  deck.sort((a, b) => {
    const oA = masterGraphemeOrderMap.get(getFocusGrapheme(a) ?? '') ?? Infinity;
    const oB = masterGraphemeOrderMap.get(getFocusGrapheme(b) ?? '') ?? Infinity;
    if (oA !== oB) return oA - oB;
    return a.word.replace(/<[^>]*>/g, '').localeCompare(b.word.replace(/<[^>]*>/g, ''));
  });

  return deck;
}

export const PHONICS_STORAGE_KEY = 'donPhonicsLastSet';
export const DEFAULT_KEYS = ['phase2_cvc|set1_satpin'];
